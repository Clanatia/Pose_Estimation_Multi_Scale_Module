...
caffe_path = '/media/vit/DATA/Tensor/caffe_train'
import sys, os
    ...
    for l in range(0, len(layername)):
        ...
        if layername[l] == 'C':
            ...
            for level in range(0,2):
                ...
                print( '%s\tch=%d\tfilter=%d\t%.1f' % (last_layer[level], outCH[l], kernel[l], lr_m))
            ...
            for level in range(0,2):
                if state == 'image':
                    #conv_name = 'conv%d_stage%d' % (conv_counter, stage)
                    conv_name[level] = 'conv%d_%d_CPM_aMulti1_L%d' % (pool_counter, local_counter, level+1) # no image state in subsequent stages
                    conv_name2[level] = 'conv%d_%d_CPM_aMulti2_L%d' % (pool_counter, local_counter, level+1) # no image state in subsequent stages
                    conv_name3[level] = 'conv%d_%d_CPM_aMulti3_L%d' % (pool_counter, local_counter, level+1) # no image state in subsequent stages
                    if stage == 1:
                        lr_m = lr_mult_distro[1]
                    else:
                        lr_m = lr_mult_distro[1]
                else: # fuse
                    conv_name[level] = 'Mconv%d_stage%d_aMulti1_L%d' % (conv_counter, stage, level+1)
                    conv_name2[level] = 'Mconv%d_stage%d_aMulti2_L%d' % (conv_counter, stage, level+1)
                    conv_name3[level] = 'Mconv%d_stage%d_aMulti3_L%d' % (conv_counter, stage, level+1)
                    lr_m = lr_mult_distro[2]
                    #conv_counter += 1
                ...
                #named = last_layer[level]
                if conv_counter == 1 :
                   conv1_named = 'Mconv%d_stage%d_WConcated_L%d' % (conv_counter, stage, level+1)
                   n.tops[conv1_named] = L.Convolution(n.tops[last_layer[level]], kernel_size=1,
                                                      num_output=outCH[l], pad=int(math.floor(1/2)),
                                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                      weight_filler=dict(type='gaussian', std=0.01),
                                                      bias_filler=dict(type='constant'))
                   last_layer[level] = conv1_named
                   print('1X1')


                n.tops[conv_name[level]] = L.Convolution(n.tops[last_layer[level]], kernel_size=3,
                                                      num_output=48, pad=int(math.floor(3/2)),
                                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*4, decay_mult=0)],
                                                      weight_filler=dict(type='gaussian', std=0.01),
                                                      bias_filler=dict(type='constant'))                
                #last_layer[level] = conv_name[level]
                n.tops[conv_name2[level]] = L.Convolution(n.tops[conv_name[level]], kernel_size=3,
                                                      num_output=40, pad=int(math.floor(3/2)),
                                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*4, decay_mult=0)],
                                                      weight_filler=dict(type='gaussian', std=0.01),
                                                      bias_filler=dict(type='constant'))
                #last_layer[level] = conv_name2[level]
                n.tops[conv_name3[level]] = L.Convolution(n.tops[conv_name2[level]], kernel_size=3,
                                                      num_output=40, pad=int(math.floor(3/2)),
                                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*4, decay_mult=0)],
                                                      weight_filler=dict(type='gaussian', std=0.01),
                                                      bias_filler=dict(type='constant'))

                #last_layer[level] = conv_name3[level]
                concat_name[level] = 'Mconv%d_stage%d_Concat_L%d' % (conv_counter, stage, level+1)
                Eltwise_name[level] = 'Mconv%d_stage%d_Eltwise_L%d' % (conv_counter, stage, level+1)

                #n.tops[share_point] = vgg network
                n.tops[concat_name[level]] = L.Concat(n.tops[conv_name[level]], n.tops[conv_name2[level]], n.tops[conv_name3[level]], concat_param=dict(axis=1))                
                n.tops[Eltwise_name[level]] = L.Eltwise(n.tops[last_layer[level]], n.tops[concat_name[level]])
                last_layer[level] = Eltwise_name[level]
                #batchnorm=1
                print( '%s\tch=%d\tfilter=%d\t%.1f' % (last_layer[level], 128, 3, lr_m))
                ...
                if layername[l+1] != 'L2' and layername[l+1] != 'L3':
                    if(state == 'image'):
                        ...
                        #ReLUname = 'relu%d_stage%d' % (conv_counter, stage)                        
                        ReLUname = 'ReLU%d_%d_CPM_L%d' % (pool_counter, local_counter, level+1)
                        n.tops[ReLUname] = L.ReLU(n.tops[v], in_place=True)
                    else:
                        if(batchnorm == 1):
                            batchnorm_name = 'Mbn%d_stage%d_L%d' % (conv_counter, stage, level+1)
                            n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer[level]], 
...
%s/build/tools/caffe train --solver=pose_solver.prototxt --gpu=0 \
--weights=/media/vit/DATA/Tensor/Pose_Estimation_Multi_Scale_Moudle/model/vgg/VGG_ILSVRC_19_layers.caffemodel \
...
if __name__ == "__main__":
    ...
    if(exp == 1):
        ...
        serverFolder = 'COCO_exp_caffe/pose56/exp22'
        base_folder = '/media/vit/DATA/Tensor/Pose_Estimation_Multi_Scale_Moudle/model'
        dataFolder = '/media/vit/DATA/Tensor/Pose_Estimation_Multi_Scale_Moudle/training/lmdb_trainVal'
        source = '/media/vit/DATA/Tensor/Pose_Estimation_Multi_Scale_Moudle/training/lmdb_trainVal'
        base_lr = 4e-5   # 2e-5
        batch_size = 5
        np = 56    # num_parts
